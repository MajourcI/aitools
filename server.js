...
app.post('/api/tool', async (req, res) => {
  ...
  try {
    ...
    if (isHumanize) {
      ...
      const t2 = strength === 'max' ? 0.97 : strength === 'light' ? 0.82 : 0.92;
...
${userText}`;
      ...
      // --- ПРОХОД 2: убийца длинных предложений + few-shot ---
      const voiceHint = sampleText ? `\n- Подстройся под манеру автора из образца:\n"""${sampleText}"""` : '';
      const sys2 = 'Ты доводишь текст до состояния, когда детекторы ИИ принимают его за написанный живым человеком. Главное оружие — рваный ритм и дробление длинных «объяснительных» предложений. Смысл не меняешь. Пишешь на русском.';
      const usr2 = `Детекторы ИИ ловят длинные ровные «объяснительные» предложения. Перепиши текст так, чтобы их не осталось.

ПРАВИЛА:
- Дроби длинные предложения на 2–3 коротких. Большинство предложений — короче 15 слов.
- Чередуй ритм: длинное предложение → совсем короткое (2–4 слова).
- Начинай предложения по-разному, в т.ч. с союзов: «А ещё», «И вот», «Но», «Именно».
- Добавляй живую текстуру: иногда риторический вопрос, короткое пояснение в скобках, лёгкую личную оценку — но без перебора${academic ? ' и без потери научности' : ''}.
- Варьируй пунктуацию: тире, скобки, иногда многоточие.
- ${academic ? 'Сохраняй термины и научность.' : 'Пиши живым человеческим языком.'}

ВОТ КАК НАДО ПЕРЕДЕЛЫВАТЬ (учись на примерах):
...
Стало (звучит как человек): «А ещё есть тёмная материя. Её примерно двадцать семь процентов от всей массы и энергии. Именно она своей гравитацией держит галактики — не даёт им разлететься.»
...
Стало: «Для астрофизиков Вселенная — это огромная лаборатория. Тут правят законы физики. Из простых атомов они собирают всё, вплоть до сложных структур.»
${voiceHint}

Теперь так же переделай весь следующий текст. Сохрани смысл, факты и примерный объём. Не используй markdown. Верни ТОЛЬКО готовый текст.
